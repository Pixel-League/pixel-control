...
LAUNCH_VALIDATE_SCRIPT="${PROJECT_DIR}/scripts/validate-dev-stack-launch.sh"
TITLEPACK_FETCH_SCRIPT="${PROJECT_DIR}/scripts/fetch-titlepack.sh"

if [ ! -f "$LAUNCH_VALIDATE_SCRIPT" ]; then
  printf '[validate-mode-launch-matrix] Missing launch validation script: %s\n' "$LAUNCH_VALIDATE_SCRIPT"
  exit 1
fi

if [ ! -f "$TITLEPACK_FETCH_SCRIPT" ]; then
  printf '[validate-mode-launch-matrix] Missing titlepack helper: %s\n' "$TITLEPACK_FETCH_SCRIPT"
  exit 1
fi

log() {
  printf '[validate-mode-launch-matrix] %s\n' "$1"
}
...
run_mode_validation_case() {
  ...
  log "Running ${case_name} launch validation"
  env "$@" bash "$LAUNCH_VALIDATE_SCRIPT"
}
...
run_mode_validation_case "joust" \
  PIXEL_SM_QA_BUILD_IMAGES=0 \
  PIXEL_SM_MODE=joust \
  PIXEL_SM_MATCHSETTINGS= \
  PIXEL_SM_TITLE_PACK=SMStorm@nadeo

run_mode_validation_case "custom" \
  PIXEL_SM_QA_BUILD_IMAGES=0 \
  PIXEL_SM_MODE=custom \
  PIXEL_SM_MATCHSETTINGS= \
  PIXEL_SM_TITLE_PACK=SMStormElite@nadeolabs

log "Elite/Siege/Battle/Joust/Custom mode launch validations passed"
