...
MATRIX_ACTION_SPECS_DIR="${PIXEL_SM_VETO_SIM_ACTION_SPECS_DIR:-${SCRIPT_DIR}/qa-veto-matrix-actions}"
...
log() {
  printf '[qa-veto-payload-sim] %s\n' "$*"
}

warn() {
  printf '[qa-veto-payload-sim][warn] %s\n' "$*" >&2
}

fail() {
  printf '[qa-veto-payload-sim][error] %s\n' "$*" >&2
  exit 1
}
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/qa-veto-payload-sim.sh status
...
Env overrides:
  ...
  PIXEL_SM_VETO_SIM_ACTION_SPECS_DIR (default: scripts/qa-veto-matrix-actions)
