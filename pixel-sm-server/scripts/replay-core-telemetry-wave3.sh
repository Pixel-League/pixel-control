...
log() {
  printf '[replay-core-telemetry-wave3] %s\n' "$1"
}
...
if [ ! -f "$ENV_FILE" ]; then
  log "Missing .env file at ${ENV_FILE}. Create it from .env.example before running wave-3 telemetry replay."
  exit 1
fi
