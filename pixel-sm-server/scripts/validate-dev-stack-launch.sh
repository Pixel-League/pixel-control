...
QA_BUILD_LOG_FILE="${QA_ARTIFACT_DIR}/launch-validation-build-${QA_TIMESTAMP}.log"
QA_STARTUP_LOG_FILE="${QA_ARTIFACT_DIR}/launch-validation-startup-${QA_TIMESTAMP}.log"
...
QA_ENV_FILE="${QA_ARTIFACT_DIR}/launch-validation-env-${QA_TIMESTAMP}.env"
...
log() {
  printf '[validate-dev-stack-launch] %s\n' "$1"
}
...
cleanup() {
  log "Stopping launch-validation stack"
...
if [ ! -f "$ENV_FILE" ]; then
  log "Missing .env file at ${ENV_FILE}. Create it from .env.example before launch validation."
  exit 1
fi
...
if [ "$QA_BUILD_IMAGES" = "1" ]; then
  log "Building images for launch validation"
...
log "Launching stack for launch validation"
...
container_log_file="${QA_ARTIFACT_DIR}/launch-validation-shootmania-${QA_TIMESTAMP}.log"
...
while [ "$(date +%s)" -le "$deadline_epoch" ]; do
  if [ -f "$runtime_log_file" ] && grep -Fq "[PixelControl] Plugin loaded." "$runtime_log_file"; then
    ...
    log "Dev stack launch validation passed"
    exit 0
  fi
  sleep 2
done
