...
log() {
  printf '[simulate-admin-control-payloads] %s\n' "$*"
}

warn() {
  printf '[simulate-admin-control-payloads][warn] %s\n' "$*" >&2
}

fail() {
  printf '[simulate-admin-control-payloads][error] %s\n' "$*" >&2
  exit 1
}
...
Usage:
  ...
  bash scripts/simulate-admin-control-payloads.sh execute <action> [key=value ...]
  bash scripts/simulate-admin-control-payloads.sh matrix [key=value ...]
